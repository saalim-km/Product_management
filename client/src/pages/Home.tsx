...
export default function ProductManagement() {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    if (selectedCategory) {
      const categorySubCategories = subCategories
        ...
        .map((sub) => sub._id)
      return categorySubCategories.includes(product.subCategory) && matchesSearch
    }
  ...
  const handleAddSubCategory = (subCategory: Omit<ISubCategory, "_id">) => {
    console.log("new sub category",subCategory);
    try {
      CategoryService.addSubCategory(subCategory.category, subCategory.name)
    } catch (error) {
      handleError(error)
    }
  ...
  const handleDeleteCategory = (categoryId: string) => {
    const categorySubCategories = subCategories.filter((sub) => sub.category === categoryId)
    ...
    setProducts(products.filter((product) => !subCategoryIds.includes(product.subCategory)))
  ...
  const handleDeleteSubCategory = (subCategoryId: string) => {
    ...
    setProducts(products.filter((product) => product.subCategory !== subCategoryId))
  ...
  const getBreadcrumbs = () => {
    const breadcrumbs = [{ label: "Home", onClick: () => setSelectedProduct(null) }]
  ...
  return (
    <>
      ...
      {/* Modals */}
      <AddCategoryModal open={showAddCategory} onOpenChange={setShowAddCategory} onAdd={handleAddCategory} />
